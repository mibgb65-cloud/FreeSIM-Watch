  ...
  <div class="public-site legal-site">
    ...
    <main id="legal-main" class="legal-shell">
      ...
      <header class="legal-heading">
        ...
        <small>生效日期：2026 年 8 月 31 日 · 版本 2026-08-31.1</small>
      </header>
      ...
      <article v-if="privacy" class="legal-document">
        ...
        <section><h2>2. 我们处理的信息</h2><ul><li><strong>LinuxDo 基础资料：</strong>启用 LinuxDo 登录时，保存用户 ID、用户名、显示名称、头像地址、信任等级和管理员角色。</li><li><strong>自托管站点 Token：</strong>只与 Cloudflare Worker Secret 中的 <code>ADMIN_TOKEN</code> 做摘要比较，不写入 D1、日志或浏览器存储。</li><li><strong>登录会话：</strong>随机会话凭据的 SHA-256 哈希、有效期及登录时间；浏览器只接收 HttpOnly Cookie。</li><li><strong>esim.gg 会话：</strong>由你主动导入的 session token。验证通过后使用 AES-GCM 加密保存，不在网页回显明文。</li><li><strong>管理员邮件配置：</strong>管理员主动添加的 Resend API Key 使用 AES-GCM 加密保存，页面只显示末四位；同时保存对应发件人、启用状态和发送错误摘要。</li><li><strong>监控与号码数据：</strong>服务端保存通知邮箱、监控参数、符合条件的发现和错误摘要。所有用户监控到的号码、最新价格和目录更新时间会汇总为全站共享目录，不记录发现该号码的用户、账号或任务。</li><li><strong>本机查询历史：</strong>只有用户点击“立即检查”时，逐轮号码结果才保存在当前浏览器的 localStorage，不上传到 D1，也不会跨设备同步。</li><li><strong>待支付订单：</strong>订单标识、Stripe 支付链接、金额、币种和错误摘要，不保存银行卡或支付密码。</li></ul></section>
        <section><h2>3. 处理目的</h2><p>这些信息仅用于验证用户身份、隔离多用户数据、运行你创建的监控任务、搜索号码历史、创建待支付订单、通过邮件发送通知、保障安全及排查故障。不使用这些数据做广告画像或出售个人信息。</p></section>
        <section><h2>4. 第三方服务</h2><ul><li><strong>LinuxDo Connect：</strong>仅在部署者启用该登录方式时完成 OAuth 登录并提供基础用户资料。</li><li><strong>Cloudflare Workers 与 D1：</strong>运行应用并保存账户、配置、密文及历史数据。</li><li><strong>esim.gg：</strong>验证你提供的会话并执行号码查询或创建待支付订单。</li><li><strong>Resend：</strong>向你配置的邮箱投递提醒。</li><li><strong>Stripe：</strong>仅在你打开待支付链接后处理付款，本工具不接收支付资料。</li></ul><p>上述服务可能按照各自政策在不同地区处理数据。</p></section>
        ...
        <section><h2>6. 安全措施</h2><p>应用采用同源请求校验、HttpOnly/Secure/SameSite Cookie、哈希会话凭据、AES-GCM 加密、按登录用户隔离查询及 Cloudflare Secret 管理关键密钥。任何系统都无法保证绝对安全；发现异常时请停止任务并重新登录或更新相关 Token 与 esim.gg 会话。</p></section>
        <section><h2>7. 你的选择与权利</h2><p>你可以删除监控任务来删除关联历史，也可以向站点部署者请求访问、更正、导出或删除账户数据，并可通过退出登录和停止使用服务撤回后续处理授权。撤回不影响此前合法完成的处理。</p></section>
        <section><h2>8. Cookie</h2><p>本站仅使用维持登录会话和 OAuth 安全校验所必需的 Cookie，不设置广告或跨站跟踪 Cookie。站点 Token 本身不会写入 Cookie。</p></section>
        <section><h2>9. 政策更新与联系</h2><p>处理目的、数据种类或第三方发生重要变化时，会更新版本并要求重新同意。隐私请求或安全问题可通过站点部署者公开的项目渠道提出；处理请求时可能需要验证当前登录身份。</p></section>
      </article>

      <article v-else class="legal-document">
        <section><h2>1. 接受协议</h2><p>勾选同意并使用站点配置的登录方式，即表示你已阅读并接受本协议与《隐私政策》。如果不同意，请不要登录、导入会话或创建监控任务。</p></section>
        ...
        <section><h2>4. 账户和安全责任</h2><ul><li>你应妥善保护 LinuxDo 账号、自托管站点 Token 和 esim.gg 账号，发现异常时立即退出并更新相关凭据。</li><li>不得上传、共享或使用他人的 Cookie、token、支付资料或其他凭据。</li><li>不得通过自动化滥用上游接口、影响其他用户或开展违法活动。</li><li>站点可以限制每用户任务数量，并可暂停明显异常或有安全风险的任务。</li></ul></section>
